...
exports.index = function(req, res) {
  ...
  if (_cityName) {
    // 通过城市名查找该城市对应的电影院
    City.findOne({ name: _cityName }).exec(function(err, searchResults) {
      if (err) {
        console.log(err);
      }
      var results = [];
    ...
    Category.findOne({ name: _fliterName })
      .populate({
        path: 'movies',
        select: 'title poster'
      })
    ...
    Category.findOne({ name: _galleryName })
      .populate({
        path: 'movies',
        select: 'title poster'
      })
    ...
    Category.find({})
      ...
      .exec(function(err, categories) {
        ...
        City.find({}).exec(function(err, cinemas) {
          if (err) {
            console.log(err);
          }
          CityProgramme.find({})
...
exports.search = function(req, res) {
  ...
  if (catId) {
    ...
    // 搜索功能
    Movie.find({ title: new RegExp(q + '.*', 'i') }) // 通过正则匹配查询电影的名称
